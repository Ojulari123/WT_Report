...
export function IngestionFigure() {
  ...
  /* Below sm the diagram is held at the width it was drawn for and the panel scrolls
     sideways. Scaled to fit a 390px screen it lands at about 0.39, which sets the 11px
     box labels at 4px and makes the figure unreadable; the columns are laid out left to
     right, so panning is the reading order anyway. From sm it scales to the column as
     before. A reader who does not pan still has the whole figure in the caption and in
     the image's own description.

     tabIndex puts the panel in the tab order, because a region that only a pointer can
     scroll is unreachable from a keyboard; axe reports exactly that as
     scrollable-region-focusable at 390px. */
  return (
    <div tabIndex={0} className="overflow-x-auto sm:overflow-x-visible">
      <svg viewBox="0 0 760 300" className="h-auto w-[47.5rem] max-w-none sm:w-full" role="img" aria-label="Architecture of the Work Term 4 agent platform. Four inbound channels on the left, Gmail, Zoho IMAP, Twilio WhatsApp and a chat webhook, all feed a single router. The router keeps a sticky session in Postgres recording the last agent per sender, so a follow-up message reaches whichever agent the user was last talking to. The router dispatches to three agents: a scheduler agent of seven workflows, a researcher agent of one workflow and a marketing agent of two workflows, ten in total. Their outputs go to Google Calendar, Google Drive, Google Sheets and WhatsApp.">
        <defs>
